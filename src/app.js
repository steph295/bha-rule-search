...
(function () {
  ...
  function adopt(data, source) {
    ...
    var changed = mergeAddedEntries();
    if (applyDeletedEntries()) changed = true;
    if (changed) { prepareEntries(state.data.entries); buildVocab(); }
    applyOverrides();
    renderStatus();
    renderTabs();
    runSearch();
  ...
  function loadOverrides() {
    fetch(OVERRIDES_URL, { cache: 'no-cache' })
      ...
      .then(function (j) {
        ...
        state.addedEntries = (j && j.addedEntries) || {};
        state.deletedEntries = (j && j.deletedEntries) || {};
        var entriesChanged = mergeAddedEntries();
        if (applyDeletedEntries()) entriesChanged = true;
        if (entriesChanged && state.data) { prepareEntries(state.data.entries); buildVocab(); }
        applyOverrides();
        applyDefinitionOverrides();
        mergeCustomDefinitions();
        if (mergeUploadedGuides() && state.guidesState === 'ready') { mergeGuides(); refreshView(); }
        if (Object.keys(state.overrides).length || entriesChanged) refreshView();
  ...
  // ---- admin-created / admin-deleted whole rules -------------------------
  //
  // rules.json is a fixed snapshot built from BHA's own source — overrides
  // can patch an existing entry's text, but can't add or drop a list item.
  // addedEntries/deletedEntries (also published to overrides.json) are the
  // extra layer that makes whole-rule creation and removal possible: an
  // added entry is keyed by a permanent id rather than its number, so
  // renumbering it later never means moving it to a new key.
  function mergeAddedEntries() {
    if (!state.data || !state.addedEntries) return false;
    var have = {};
    state.data.entries.forEach(function (e) { if (e._addedId) have[e._addedId] = true; });
    var added = false;
    Object.keys(state.addedEntries).forEach(function (id) {
      if (have[id]) return;
      var a = state.addedEntries[id];
      state.data.entries.push({
        kind: 'manual', code: id, letter: a.letter, num: a.num, doc: a.doc,
        title: a.title, html: a.html, path: a.path || [], isNew: false, _addedId: id
      });
      added = true;
    });
    return added;
  ...
  function applyDeletedEntries() {
    if (!state.data || !state.deletedEntries || !Object.keys(state.deletedEntries).length) return false;
    var deleted = state.deletedEntries;
    var before = state.data.entries.length;
    state.data.entries = state.data.entries.filter(function (e) { return !deleted[entryKey(e)]; });
    return state.data.entries.length !== before;
  }
  ...
  function applyOverrides() {
    ...
    state.data.entries.forEach(function (e) {
      // An added entry's own content lives in addedEntries (it has no BHA
      // original to patch against), everything else in the usual overrides
      // patch layer — same {title, html, flag} shape either way.
      var o = e._addedId ? (state.addedEntries && state.addedEntries[e._addedId]) : state.overrides[entryKey(e)];
      if (!o) return;
